	...
	try {
		...
		if (streamResponse) {
			...
			for await (const chunk of stream) {
				let tmpContent = chunk.choices[0]?.delta?.content || '';
				let content = '';

				if (tmpContent === "<think>") {
					isThinking = true;
					continue;
				} else if (tmpContent === "</think>") {
					isThinking = false;
					continue;
				} else if (!isThinking) {
					content = tmpContent;
			...
			let fullResponse = typeof response === 'string' 
...
Rephrased search query:`;
	...
	if (typeof response === 'string' && response.includes('choices')) {
		try {
			...
			console.error('Failed to parse response string to object:', JSON.stringify(error, null, 2));
			throw error;
		}
	}
	let searchQuery;
	if (responseObj.error) {
		...
		searchQuery = responseObj.choices[0].message.content.trim();
		// remove <think> tags
		const regex = /<think>[.\S\n\s]*<\/think>/gm;
			...
			if (trimmedContent) {
				...
				if (process.env.CHUNK_CONTENT === 'true') {
					...
					if (process.env.CHUNK_CONTENT_USE_HYBRID_FALLBACK === 'true' && (!summarizedContent || summarizedContent.trim().length === 0)) {
						summarizedContent = await summarizeContent(trimmedContent, parseInt(process.env.CHUNK_CONTENT_MAX_TOKEN_SIZE) || 500);
					}
				} else {
					// otherwise, use a simple truncation method to summarize the content
					summarizedContent = await summarizeContent(trimmedContent, process.env.WEB_PAGE_CONTENT_MAX_LENGTH);
				}
...
/* ------------------------------------ */\n\n`;
		...
		await fs.writeFile(`${__dirname}/error_log.txt`, `${new Date().toISOString()}: ${error}\n`, 'utf8');
...
async function summarizeContent(content, maxLength = 1000) {
	const sentences = await parseSentences(content);
